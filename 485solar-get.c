...
int print_help(char solar_get_version[7], char version_date[11] ) {
  ...
  printf("%s %s %s\n\n", solar_get_version, version_date, "by roland@breedveld.net updated by lmarabini@gmail.com"); 
...
{
  char solar_get_version[7]="v1.200";
  char version_date[11]="2019-11-29";
  solar_get_version[7]='\0';
  version_date[11]='\0';
  int i,j;
...
*/
  ...
  nr_of_devices=device_nr; 
  struct inverter inv_file[nr_of_devices];
  int id_mapping[nr_of_devices];
  int id_mapping_done = 0;
  ...
  if(init_comm==1){
	...
	if (id_mapping_done == 0){
		devices_list_file = fopen("/etc/devices/devlist.bin", "rb");
		if(show_debug==1) fprintf(stderr,"number of devices: %d\n", nr_of_devices);
		if (devices_list_file == NULL){
			if(show_debug==1) fprintf(stderr,"building mapping file...\n");
			DWORD inv_sn;
			if(show_debug==1) fprintf(stderr,"getting inverters sn...\n");
			for(i=0;i<nr_of_devices+1;i++){
				if(show_debug==1) fprintf(stderr," - inverter %d: ", i);
				inv_file[i].id = i;
				GetDeviceSN( DeviceHandle[i], &inv_sn);
				inv_file[i].sn = inv_sn;
				id_mapping[i] = i;
				if(show_debug==1) fprintf(stderr,"ID %d SN %d\n", i, inv_sn);
			}
			devices_list_file = fopen("/etc/devices/devlist.bin", "wb");
			fwrite (&inv_file, sizeof(struct inverter), nr_of_devices+1, devices_list_file);
			if(show_debug==1) fprintf(stderr,"print detected info:\n");
			for(i=0;i<nr_of_devices+1;i++){
				if(show_debug==1) fprintf(stderr, "ID %d SN %d\n", inv_file[i].id, inv_file[i].sn);
			}
		}else{
			fread(&inv_file, sizeof(struct inverter), nr_of_devices+1, devices_list_file);
			if(show_debug==1) fprintf(stderr,"print info from file:\n");
			for(i=0;i<nr_of_devices+1;i++){
				if(show_debug==1) fprintf(stderr,"ID %d SN %d\n", inv_file[i].id, inv_file[i].sn);
				DWORD inv_sn = 0;
				for(j=0;j<nr_of_devices+1;j++){
					GetDeviceSN(DeviceHandle[j], &inv_sn);
					if (inv_file[i].sn == inv_sn){
						id_mapping[i] = j;
						break;
					}
				}
		...
		fclose(devices_list_file);
		id_mapping_done = 1;
		for(i=0;i<nr_of_devices+1;i++){
			DWORD inv_sn;			
			if(show_debug==1){
				GetDeviceSN(DeviceHandle[id_mapping[i]], &inv_sn);
				fprintf(stderr, "mapping inv nr. %d (sn %d) from file to id %d (sn %d) detected\n", i, inv_file[i].sn, id_mapping[i], inv_sn);
			}
		}
	}
    ...
    if (ires) {
      while (exit_loop==0) {
        ...
        if(show_data==1){
		  ...
		  device_nr = id_mapping[device_nr];
